...
void _rotacion::_rotacion_ply(char *archivo, int num){
	...
	parametros(ply_rot,num,false);
}


void _rotacion::parametros(vector<_vertex3f> perfil, int num, bool cono)
{
int i=0,j;
...
if (perfil[0].x == 0.0, perfil[0].z == 0.0, perfil[0].y != 0.0){
	eje1 = perfil[0];
	//perfil.erase(perfil.begin,perfil.begin);
}

if (perfil[num_aux-1].x == 0.0, perfil[num_aux-1].z == 0.0, perfil[num_aux-1].y != 0.0){
	eje2 = perfil[num_aux-1];
	//perfil.removeElementAt(num_aux-1);
}

for (j=0;j<num;j++) {
	for (i=0;i<num_aux;i++) {
	  if (perfil[i] == eje1 || perfil[i] == eje1){}
	  else{
  ...
  if (eje1.y != 0.0){
  	vertices.push_back(eje1);
  }

  if (eje2.y != 0.0){
  	vertices.push_back(eje2);
  }
/*
  if (cono){
	vertices.push_back( _vertex3f(0.0, 1.5, 0.0) );
  }*/
